...
class KubernetesClient:
    def __init__(self, namespace: str, client: k8s_client.CoreV1Api | None = None):
        ...
        if client:
            ...
            try:
                ...
                logging.warning("Error loading kube config: %s", e)
    ...
    def _wait_for_deployment_ready(self, name: str, namespace: str, timeout: int = 60):
        ...
        while True:
            deployment = apps_client.read_namespaced_deployment_status(
                        ...
                        "containers": [
                            {
                                ...
                                "args": [
                                    "sh",
                                    "-c",
                                    "dig +short localhost.localstack.cloud | grep -qv 127.0.0.1",
                                ],
                            }
                        ],
                        "restartPolicy": "Never",
...
def create_fn(body, **kwargs):
    ...
    if release_name != "localstack":
        return


    logging.info("LocalStack service created")
...
def update_fn(body, **kwargs):
    ...
    logging.info("LocalStack service updated")
    namespace = annotations.get("meta.helm.sh/release-namespace")
    client = KubernetesClient(namespace)
    ip = client.get_localstack_service_ip_address()
    client.update_dns_configuration(ip)
...
def delete_fn(body, **kwargs):
    ...
    logging.info("LocalStack service deleted")
    namespace = annotations.get("meta.helm.sh/release-namespace")
    client = KubernetesClient(namespace)
    client.reset_dns_configuration()
